...
class InlineConstsContext(lowerings.context.LoweringContextPlugin):
    ...
    constant_cache: dict[object, ir.Attribute] = dataclasses.field(default_factory=dict)

    @classmethod
    def get(cls, lctx: lowerings.context.LoweringContext) -> "InlineConstsContext":
...
def _tensor_fingerprint(tensor: torch.Tensor) -> tuple[str, object, object, object]:
    return (
        str(tensor.device),
        tensor.shape,
        tensor.stride(),
...
def tensor_lowering_placeholder(*args, **kwargs):  # pylint: disable=g-doc-args
    ...
    """
    del args, kwargs
    raise RuntimeError("tensor_lowering_placeholder should not be called directly.")
...
def inline_consts(exported_program: torch.export.ExportedProgram) -> None:
    ...
    for i, (tensor, node) in enumerate(zip(flat_consts, exported_program.graph.nodes)):
