#!/usr/bin/env bash
# Attempt 49: v9_0_3 + import_forever=false + FC bias NOP + MapReshapeOps NOP
...
LOG_FILE="$ROOT/tmp/att49-v9-shim.log"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TOOLS_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
PROTECT_SO=/tmp/protect_dla_dir.so

exec > >(tee -a "$LOG_FILE") 2>&1
echo "=== Attempt 49: v9_0_3 + import_forever=false + FC bias NOP + MapReshapeOps NOP + shim ==="
date -Is
